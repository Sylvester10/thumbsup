...
class Staff_model_ajax extends CI_Model {
	...
	public function actions($staff_id) {
		...
		if ($y->active != 'true') {
			$activate_action = '<p><a type="button" href="' . base_url('school_staff/activate_staff/'.$staff_id) .'" class="btn btn-default btn-sm btn-block action-btn clickable"> <i class="fa fa-check" style="color: green"></i> &nbsp; Activate Staff </a></p>';
		} else {
			$activate_action = '<p><a type="button" href="' . base_url('school_staff/deactivate_staff/'.$staff_id) .'" class="btn btn-default btn-sm btn-block action-btn clickable"> <i class="fa fa-times" style="color: red"></i> &nbsp; De-activate Staff </a></p>';
		}
		...
		<p><a type="button" href="' . base_url('school_staff/edit_staff/'.$staff_id) .'" class="btn btn-default btn-sm btn-block action-btn clickable"> <i class="fa fa-pencil" style="color: green"></i> &nbsp; Edit Staff </a></p>'

		. $activate_action .

		'<p><a type="button" href="#" class="btn btn-default btn-sm btn-block action-btn clickable" data-toggle="modal" data-target="#message'.$staff_id.'"> <i class="fa fa-envelope" style="color: green"></i> &nbsp; Message Staff </a></p>
	...
	public function modals($staff_id) {
		$y = $this->common_model->get_staff_details_by_id($staff_id);
		$modal_message_user = modal_message_user($id, $y->name, 'school_staff/message_staff');

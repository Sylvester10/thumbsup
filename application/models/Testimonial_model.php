...
class Testimonial_model extends CI_Model {
	...
	public function add_new_testimonial($photo, $photo_thumb) {
		$name = ucwords($this->input->post('name', TRUE));
		$position = ucwords($this->input->post('position', TRUE));
	...
	public function edit_testimonial($testimonial_id, $photo, $photo_thumb) {
		$name = ucwords($this->input->post('name', TRUE));
		$testimony = ucfirst($this->input->post('testimony', TRUE));
		$position = ucwords($this->input->post('position', TRUE));
		$date = time();
	...
	public function delete_testimonial_photo($testimonial_id) {
		$y = $this->get_testimonial_details($testimonial_id);
		if ($y->photo != NULL && $y->photo_thumb != NULL) {
			unlink('./assets/uploads/testimonials/'.$y->photo); //delete the photo
			unlink('./assets/uploads/testimonials/'.$y->photo_thumb); //delete the thumbnail
		}
    } 


	public function delete_testimonial($testimonial_id)	{
		$y = $this->get_testimonial_details($id);
		$this->delete_testimonial_photo($id); //remove image files from server

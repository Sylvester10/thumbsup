...
class Admin extends MY_Controller {
	...
	public function index() { //admin dashboard, routed as dashboard
		...
		$data['profiles'] = count($this->common_model->get_profile());
		$data['volunteer'] = count($this->volunteer_model->get_volunteers());
		$data['structures'] = count($this->org_structure_model->structures());
		$data['units'] = count($this->units_model->get_all_units());
		$this->load->view('admin/dashboard/dashboard', $data); 
		$this->admin_footer();
	}

	public function test() {
		$email = 'onyekaesso10@gmail.com';	
		$subject = 'Test Message';
		$message = 'This is a test message';

		$status = $this->email_model->send_mail($email, $subject, $message);
		$status = ($status)? 1 : 0 ;
		echo $status;	//indicator of success, will be used to check status in javascript
